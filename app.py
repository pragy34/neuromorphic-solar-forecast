...
def build_scaler_and_window(city: str):
    ...
    avail_feats = [f for f in SEQUENCE_FEATURES
    ...
    df_sc       = pd.DataFrame(
        scaler.transform(df[all_cols].values),
...
def load_model(city: str, horizon: int,
                n_features: int) -> NeuroSpikeSNN:
    path  = os.path.join(CKPT_DIR, f"snn_{city}_h{horizon}.pt")
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Model not found: snn_{city}_h{horizon}.pt"
        )
    model = NeuroSpikeSNN(
...
def get_metrics():
    ...
    results = {}
    for fname in ["baseline_results.csv",
                   "lstm_results.csv",
                   "snn_results.csv"]:
...
def forecast(req: ForecastRequest):
    ...
    if city not in config.CITIES:
        raise HTTPException(
            ...
            detail=f"City '{city}' not supported. "
                   f"Use /cities to see available options."
        )
    if horizon not in HORIZONS:
        raise HTTPException(
            status_code=400,
            detail=f"Horizon {horizon} not supported. "
                   f"Use one of {HORIZONS}."
        )

    try:
        last_window, scaler, n_features, last_time = (
            build_scaler_and_window(city)
        )
    ...
    timestamps = [
        str(last_time + pd.Timedelta(hours=h + 1))
